...
public abstract class BaseListFragment<T> extends BaseFragement implements BaseQuickAdapter.RequestLoadMoreListener {
    ...
    protected void initView(View view) {
        mRecyclerView = (RecyclerView) view.findViewById(R.id.base_list_rv);
        initItemLayout();
        chooseListType(mListType, mIsVertical);
        if (-1 == layoutResId) {
            throw new RuntimeException("layoutResId is null!");
        }
        mAdapter = new CblListAdapter(layoutResId, new ArrayList<>());
        initSetting();
    ...
    public class CblListAdapter extends BaseQuickAdapter<T, BaseViewHolder> {
        ...
        CblListAdapter(int layoutResId, List<T> data) {
